...
class DLA34(nn.Module):
    ...
    def _make_conv_level(inplanes, planes, convs, stride=1):
        ...
        for i in range(convs):
            modules.extend([nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride if i == 0 else 1, padding=1, bias=False),
...
class DeformConv(nn.Module):
    def __init__(self, chi, cho):
        super().__init__()
        self.actf = nn.Sequential(nn.BatchNorm2d(cho, momentum=0.1),
                                  nn.ReLU(inplace=True))
...
class DLAUp(nn.Module):
    def __init__(self, startp, channels, scales, in_channels=None):
        ...
        for i in range(len(channels) - 1):
            j = -i - 2
            setattr(self, f'ida_{i}', IDAUp(channels[j], in_channels[j:], scales[j:] // scales[j]))
            scales[j + 1:] = scales[j]
            in_channels[j + 1:] = [channels[j] for _ in channels[j + 1:]]

    def forward(self, layers):
        out = [layers[-1]]  # start with s = 32
...
class DLASeg(nn.Module):
    def __init__(self, heads, pretrained, final_kernel, head_conv, out_channel=0):
        super().__init__()
        ...
        for head in self.heads:
            ...
            fc = nn.Sequential(nn.Conv2d(channels[self.first_level], head_conv, kernel_size=3, padding=1, bias=True),
                               nn.ReLU(inplace=True),
